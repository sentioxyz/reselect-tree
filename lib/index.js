...
const { createSelector, createStructuredSelector, createSelectorCreator, defaultMemoize } = require("reselect");
...
class Leaf {
  constructor(deps, selector, equalityCheck) {
    this.deps = deps;
  ...
  contextualize(resolve, pointer) {
    ...
    if (this.equalityCheck) {
      return createSelectorCreator(defaultMemoize, this.equalityCheck)(resolved, this.selector);
    }
    return createSelector(resolved, this.selector);
  }
}

export function createLeaf(deps, selector, equalityCheck) {
  return new Leaf(deps, selector, equalityCheck);
}
